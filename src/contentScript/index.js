...
async function init() {
  ...
  if (isEnabled) {
    startObserving();
    // Add polling to catch any missed messages or those that timed out previously
    setInterval(checkExistingMessages, 2000);
  }
}
...
const processingTextElements = new WeakSet();
...
async function processMessageContainer(container) {
  ...
  // Check if this text element is already processed or processing
  // This handles cases where multiple containers map to the same text
  if (processedTextElements.has(textElement) || processingTextElements.has(textElement)) {
    // Mark container as processed too so we don't keep checking it
  ...
  processingTextElements.add(textElement);

  console.log('ChatGPT Gemini Modifier: Processing message');

  // Hide immediately and show streaming indicator
  addStreamingIndicator(textElement);
  ...
  if (!isComplete) {
    ...
    processingTextElements.delete(textElement);
    // Restore visibility if we timed out
  ...
  processedMessages.add(container);
  processingMessages.delete(container);
  // Keep in processingTextElements until we are done or added to processedTextElements
...
async function waitForResponseCompletion(element, maxWaitTime = 180000) {
...
function addStreamingIndicator(element) {
  // Check if indicator already exists
  if (element.parentNode?.querySelector('.gemini-modifier-loading')) return;
